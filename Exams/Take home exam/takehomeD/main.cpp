...
//arithmos mitroou%100=14
//14%4=2 ara linw tin D
...
public:
    ...
    //sto public pedio oi mones prostheseis einai i pio katw sinartisi
    Iterator<T> rank(const T &x) {
        if(x<0 || x>=the_size){return end();}//an einai ektos oriwn
        //epistrefw end();
        //alliws epistrefo tin sinartisi mou
        return Iterator<T>(new TreeIteratorImpl(rank(root, x)));
    }
...
private:
    ...
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    //sto private pedio exo prosthesei mono tin rank(node* int) kai
    //rank_help(node* int int bool node*&)
    static node *rank(node *t,const int &x){
        if(x==0){return leftdown(t);}//an thelei to pio mikro
        int target=0;
        node *vans;
        bool flag=true;
        rank_help(t,target,x,flag,vans);//o(n)
        return vans;
    }
    /*
     * stin ousia i rank help einai mia inorder diasxisi
     * i opoia afksanei tin metavliti tis rank_position
     * kathe fora pou diavazei ena komvo, an vrei to target
     * diladi to x tha mpei sto if mono mia fora (logo tou flag)
     * alliws tha epistrefei sinexia xwris na proxoraei tin metavliti
     * epistrefontas sto telos tin metavliti ans i opoia einai & gia
     * na tin epistrepsei i rank apo panw
     * */
    static void rank_help(node* t,int &rank_position,const int &targ,bool &flag,node* &ans){
        if(t!=nullptr){
            rank_help(t->left,rank_position,targ,flag,ans);
            if(rank_position!=targ){
                ++rank_position;
            }else{
                if(flag){
                    flag=false;
            ...
            rank_help(t->right,rank_position,targ,flag,ans);
        }
    }
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
